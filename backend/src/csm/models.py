...
class ModelArgs:
    ...
    def items(self):
        """Make ModelArgs dict-like for PyTorchModelHubMixin compatibility."""
...
class Model(
    ...
    def __init__(self, config: ModelArgs = None, **kwargs):
        super().__init__()
        
        # Debug: print what we received
        print(f"Model.__init__ called with config={config}, kwargs keys={list(kwargs.keys())}")
        
        # Handle config passed as dict from HuggingFace Hub
        if config is None and not kwargs:
            raise ValueError("Model config is required")
        
        if config is None:
            # Check if config is nested under a key
            if 'model_args' in kwargs:
                config = ModelArgs(**kwargs['model_args'])
            elif 'config' in kwargs:
                cfg = kwargs['config']
                if isinstance(cfg, dict):
                    config = ModelArgs(**cfg)
                else:
                    config = cfg
            else:
                # Config passed directly as kwargs
